...
function run() {
  ...
  button.onclick = function() {
    ...
    let values;
    try {
      values = JSON.parse(scene.value);
    } catch(e) {
      alert(`invalid json: ${e}`);
      return
    }
    canvas.width = values.width;
    canvas.height = values.width * 2 /3;
    render(new Scene(), values);
  };
...
function render(scene, values) {
  if (rendering) {
    rendering.stop();
    rendering = null;
  }
  rendering = new State(scene.render(values.width, values.samples_per_pixel, values.max_depth,parseInt(concurrency.value), pool));
}
